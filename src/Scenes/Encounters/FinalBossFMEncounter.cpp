#include "FinalBossFMEncounter.hpp"
...
namespace HJ { namespace Encounters {
	...
	FinalBossFMEncounter::FinalBossFMEncounter(GameDataRef t_data)
	...
	void FinalBossFMEncounter::Init()
	{
		...
		m_activeHeroes.push_back(m_data->gm.hSorceress);
		...
		auto bgSprite = bg->AddComponent<SpriteComponent>("C_FrostMageBattleBGSprite");
		// define bg sprite
		bgSprite->GetSprite().setTexture(m_data->assets.GetTexture("Tex_FrostMageBattleBG"));
		...
		int health = m_data->gm.hKnight->GetMaxHealth() * 4;
		...
		// Add skills
		m_activeBoss->GetSkillComponent()->AddSkill(SKILL_NAME::SPECIAL_SKILL_1, std::make_shared<FrostBolt>());
		m_activeBoss->GetSkillComponent()->AddSkill(SKILL_NAME::SPECIAL_SKILL_2, std::make_shared<FrostRing>());
		m_activeBoss->GetSkillComponent()->AddSkill(SKILL_NAME::RAGE, std::make_shared<FrostArmor>()); ///> rage skill!
		...
		// Text indicating bard stats
		auto sorcStatsTxt = std::make_shared<Entity>();
		auto sorcStatsTxtComp = sorcStatsTxt->AddComponent<TextComponent>("C_HeroStatsText");
		sorcStatsTxtComp->GetText().setFont(m_data->assets.GetFont("Font_Pixel"));
		sorcStatsTxtComp->GetText().setCharacterSize(14);
		heroHP = std::to_string(m_data->gm.hSorceress->GetHealth()) + "/" + std::to_string(m_data->gm.hSorceress->GetMaxHealth()) + "HP";
		heroMP = std::to_string(m_data->gm.hSorceress->GetMana()) + "/" + std::to_string(m_data->gm.hSorceress->GetMaxMana()) + "MP";
		sorcStatsTxtComp->GetText().setString(m_data->gm.hSorceress->className() + ": " + heroHP + " | " + heroMP);
		sorcStatsTxtComp->GetText().setColor(sf::Color::White);
		sorcStatsTxt->SetPosition(sf::Vector2f(SCREEN_WIDTH * 0.035f, SCREEN_HEIGHT * 0.85f));
		sorcStatsTxt->SetAlive(true);
		sorcStatsTxt->SetVisible(true);
		sorcStatsTxt->Init();
		...
		m_statistics.push_back(sorcStatsTxt);
		...
		AddEntity("E_HeroSorceress", m_data->gm.hSorceress);
		...
		AddEntity("E_aSorceressStatsTxt", sorcStatsTxt);
